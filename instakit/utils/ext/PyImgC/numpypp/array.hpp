...
#ifndef NUMPYPP_ARRAY_HPP_
#define NUMPYPP_ARRAY_HPP_
...
namespace numpy {
    ...
    struct position {
        ...
        bool operator == (const position& other) {
            return !std::memcmp(
                this->position_,
                other.position_,
                sizeof(this->position_[0]) * this->nd_);
            }
        ...
        static
        position from2(npy_intp p0, npy_intp p1) {
            position res;
            res.nd_ = 2;
            res.position_[0] = p0;
            res.position_[1] = p1;
        ...
        static
        position from3(npy_intp p0, npy_intp p1, npy_intp p2) {
            position res;
            res.nd_ = 3;
            res.position_[0] = p0;
            res.position_[1] = p1;
    ...
    struct position_vector {
        ...
        // meaningless. Therefore, we want to only store the correct number of dimensions.
        // This structure manages this.
        ...
        public:
            ...
            void push_back(const position& p) {
                ...
                for (int d = 0; d != size_; ++d) {
                    store_.push_back(p[d]);
                }
            }

            unsigned size() const {
                return store_.size() / size_;
            }
    ...
    struct position_queue : protected position_vector {
        ...
        unsigned size() const {
            return this->position_vector::size() - next_;
        }
    ...
    struct iterator_base : std::iterator<std::forward_iterator_tag, BaseType> {
...
#endif
            BaseType* data_;
            // steps is similar to strides, but more useful for iteration -- 
            // see implementation of the `++` operator [below] ...
            // Also, note that I divide by sizeof(BaseType)
            int steps_[NPY_MAXDIMS];
            int dimensions_[NPY_MAXDIMS];

            // This is actually *not* the position we are at -- rather it's the reverse (!)
        ...
        public:
            ...
            int index(unsigned i) const {
                return index_rev(position_.nd_ - i - 1);
            }

            int index_rev(unsigned i) const {
                return position_.position_[i];
            }

            npy_intp dimension(unsigned i) const {
                return dimension_rev(position_.nd_ - i - 1);
            }

            npy_intp dimension_rev(unsigned i) const {
            ...
            std::ostream& operator << (std::ostream& out, const iterator_base& iter) {
                /// WHAT THE FRIG DOES THAT EVEN MEAN, "FRIEND" ?!???!?! ...
                /// really sometimes I find that I do NOT understand C++'s game AT ALL
                ///     -fish
            ...
            bool is_valid() const {
                ...
                for (int r = 0; r != p.ndim(); ++r) {
                    if (p[r] < 0 || p[r] >= PyArray_DIM(base, r)) {
                        return false;
                    }
                }
#endif
    ...
    /// You know there are some people out there who consider
    /// themselves LUCKY to even HAVE const, missy
    template<typename T>
    struct no_const {
        typedef T type;
    };
    ...
    class iterator_type : public iterator_base<BaseType> {
        ...
        public:
            ...
            BaseType operator * () const {
                ...
                /// I am not awesome enough with templates
                /// to really follow allong on this next line:
    ...
    class array_base {
        ...
        public:
            array_base(const array_base<BaseType>& other)
                :array_(other.array_) {
                    if (sizeof(BaseType) != PyArray_ITEMSIZE(array_)) {
                        std::cerr   << "mahotas:" << __PRETTY_FUNCTION__
                                    << " mix up of array types"
                                    << " [using size " << sizeof(BaseType)
                                    << " expecting " << PyArray_ITEMSIZE(array_)
                                    << "]\n";
                        assert(false);
                    }
                    Py_INCREF(array_);
            ...
            /// I LIKE MY TYPES LIKE I LIKE MY WOMEN:
            /// POLYMORPHICALLY TYPEDEF'D TO INDEXES
            /// DUDE BRO AMIRITE
            index_type size() const {
                return PyArray_SIZE(array_);
            }
            ...
            PyArrayObject* raw_array() const {
                return array_;
            }

            void* raw_data() const {
                return PyArray_DATA(array_);
            }

            const npy_intp* raw_dims() const {
                return PyArray_DIMS(array_);
            }
    ...
    struct array : public array_base<BaseType> {
        ...
        public:
            ...
            /// NOTE TO SELF: seriously read up on c++ iterators and shit
            iterator begin() {
    ...
    /// Fuck, I should have saved the "I LIKE MY WOMEN LIKE" joke until I got down here,
    /// to all these `aligned_array` declarations as it would have been waaaaaaaay hillariouser
    ///     -fish
    ...
    inline
    bool are_arrays(PyArrayObject* a) {
        return PyArray_Check(a);
    }

    /// Due to years and years of writing bash scripts, and before that shorthand perl,
    /// these next two funcs' use of logical-and operators looks like the second call
    /// to `PyArray_Check()` is supposed to happen after `return` successfully executes...
    /// wow yeah, this code's elegance is above my pay grade in places. Fuck me I'm low-class

    inline
    bool are_arrays(PyArrayObject* a, PyArrayObject* b) {
    ...
    inline
    bool equiv_typenums(PyArrayObject* a, PyArrayObject* b) {
        return PyArray_EquivTypenums(PyArray_TYPE(a), PyArray_TYPE(b));
    }

    inline
    bool equiv_typenums(PyArrayObject* a, PyArrayObject* b, PyArrayObject* c) {

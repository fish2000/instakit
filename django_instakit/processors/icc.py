...
from django_instakit.utils.dicts import OrderedDict, ADict, AODict
from django_instakit.utils.encoding import get_encodings
from django_instakit.utils import spectralarithmetic
...
# where they at.
class ICCProfilesHome(object):
    def path(self, *pth):
        from django.conf import settings
        return os.path.join(
            settings.STATIC_ROOT,
            'django_instakit', 'icc', *pth)
    def __repr__(self):
        return self.path()
    def __str__(self):
...
iccprofiles_home = iccprofiles = ICCProfilesHome()
...
class VideoCardGammaTableType(VideoCardGammaType):
    ...
    def getFormulaType(self):
        """ Return formula representing gamma value at 50-percent input. """
...
class ICCProfileTransform(object):
    def __init__(self,
        source=None,
        mode="RGB",
...
class ICCProfile(object):
    ...
    @property
    def data(self):
        """ Get raw binary profile data. This will re-assemble the profile
            from its constituent parts (header, tag table and data) on-the-fly. """
    ...
    def isSame(self, profile, force_calculation=False):
        """ Compare the ID of profiles.
            ...
            The `profile` can be a ICCProfile instance, a binary string
    ...
    def write(self, stream_or_filename=None):
        """ Write profile to stream, re-assembling the data from its parts --
            header, tag table and data -- on-the-fly. """
    ...
    def process(self, img):
        """ ICCProfile objects are ImageKit processors,
            by virtue of their implementation of this method. """
        return img
